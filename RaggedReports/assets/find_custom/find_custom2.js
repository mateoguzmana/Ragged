...
var find_root_node = "accoridionContentDiv"; // Leave as null to search entire doc or put id of div to search (ex: 'content'). Ver 5.0a - 7/18/2014
...
function findit() 
{
        ...
        //$(".reference-collapse").collapse("show");
	...
	else
	{
		...
		setTimeout(function(){$(".reference-collapse").collapse("hide");
		...
		else
		{
			...
			find_pointer = -1;	
		}}, 351); // highlight all occurrences of search string                
                
                $(".reference-collapse").collapse("hide");
                if (highlights.length > 0) // if we found occurences
		{
			find_pointer = -1;
			findnext(); // Find first occurrence
...
function findnext()
{
	...
	if(find_msg != undefined && find_msg != null){
	...
	if (found_selected_rule == 1)
        ...
        var span =  current_find.id;
        var idToShow = $("#"+span).parent().attr("data-inner");
        $(".reference-collapse").collapse("hide");
        
        
        
        setTimeout(function(){
            $(".isClicked").collapse("show");
            $("#"+idToShow).collapse("show");},351);
...
// This part creates the instructions and the "find" button
	...
	+ '<input id="btn" type="button" value="Siguiente" onclick="this.blur(); findit();">' // ver 5.3 - 5/15/2015 - added this.blur();
...
*/
var find_content = '<div id="window_body" style="padding: 0px;">'
+ '<label for="profile">Buscar en p\u00C1gina:</label>'
+ '<form onsubmit="return false;"><input class="form-control" type="search" size="25" maxlength="25" id="fwtext"'
+ ' onchange="resettext();">'
+ '<input style="width:50%" class="btn btn-default" type="button" value="Anterior" onclick="findprev();">'
+ '<input style="width:50%" id="btn" class="btn btn-default" type="button" value="Siguiente" onclick="findit();">'
+ '</form></div>'
+ '<div id="find_msg"><br /></div>';
//document.write(find_content);

$("#finder").html(find_content);
